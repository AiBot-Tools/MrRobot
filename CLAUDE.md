...
Phase 0 scaffold, delivered as a repo: `tsc --noEmit` clean, 19/19 `node:test` passing, kernel boots degraded with no pmmcp and no Docker. See README.md "What is real, what is a stub". Stubs and gaps: Apple container driver (stub), egress proxy sidecar (absent), cron scheduler (absent), restart-safe projections for approvals/quarantine (absent), live tests for runtime loop and Docker driver (absent), `SecretsBroker.keyArg` unverified.
